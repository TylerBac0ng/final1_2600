...
make_move() {
    # Only move if it's O's turn and game is active
...
mosquitto_sub -h $SERVER -t $TOPIC_STATE -t $TOPIC_RESULT -q 1 -i "$CLIENT_ID" | while read -r LINE; do
